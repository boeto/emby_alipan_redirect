...
class AliyunPan(metaclass=Singleton):
    def __init__(self):
        ...
        self.alipan_directory_name = str(
            configs["sync"]["alipan_directory_name"]
        )
        self.emby_directory = str(configs["sync"]["emby_directory"])

        # 不存在这个配置了，未改对应的逻辑
        self.dav_source_directory = str(
            configs["sync"]["dav_source_directory"]
        )
        self.dav_url = str(configs["sync"]["dav_url"])
        ...
        # 本次文件存储路径
        self.__folder_json = "/mnt/data/folder_files.json"
        # 文件夹->文件映射关系
        self.__folder_files = {}
        # 读取已有本地文件
        if Path(self.__folder_json).exists():
            logger.info(f"读取已有本地文件 {self.__folder_json}")
    ...
    def __get_folder_files(self, parent, file):
        ...
        if (
            file
            and file.type == "file"
            and Path(file.name).suffix in self.RMT_MEDIA
        ):
            path = Path(parent).joinpath(file.name)
            path = str(path).replace(
                self.alipan_directory_name, self.emby_directory
            )
            path = path.replace("\\", "/")
    ...
    def sync_aliyunpan(self):
        ...
        # 读取已有本地文件
        if Path(self.__folder_json).exists():
            logger.info(f"读取已有本地文件2::: {self.__folder_json}")
            with open(self.__folder_json, "r") as file:
                content = file.read()
                if content:
                    self.__folder_files = json.loads(content)
        ...
        if self.__folder_files:
            ...
            file.close()
            logger.info(f"写入本地文件完成 {self.__folder_json}")
        else:
            logger.warning("未获取到文件列表")
    ...
    def save_new_file_id(self, dest_dir):
        ...
        :return:
        """
        alipan_dir = str(dest_dir).replace(
            self.emby_directory, self.alipan_directory_name
        )
    ...
    def get_download_url(self, dest_dir: str):
        ...
        logger.info(f"straight_chain::: {self.straight_chain}")
        ...
        if not self.straight_chain or not file_id:
            ...
            # 对盘符之后的所有内容进行url转码
            dest_dir = self.dav_source_directory + dest_dir
            dest_dir = urllib.parse.quote(str(dest_dir), safe="")
            dest_dir = f"http://{self.dav_url}/d/{dest_dir}"
            logger.info(f"获取dav播放链接 {dest_dir}")
            return dest_dir
        else:
